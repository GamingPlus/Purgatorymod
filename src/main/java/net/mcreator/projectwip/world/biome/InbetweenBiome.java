...
public class InbetweenBiome extends InbetweendimensionModElements.ModElement {
	@ObjectHolder("inbetweendimension:inbetween")
	public static final CustomBiome biome = null;
	public InbetweenBiome(InbetweendimensionModElements instance) {
		super(instance, 1);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		BiomeDictionary.addTypes(biome, BiomeDictionary.Type.WASTELAND, BiomeDictionary.Type.HILLS, BiomeDictionary.Type.FOREST,
				BiomeDictionary.Type.HOT, BiomeDictionary.Type.DEAD, BiomeDictionary.Type.SPOOKY, BiomeDictionary.Type.DENSE,
				BiomeDictionary.Type.MOUNTAIN, BiomeDictionary.Type.HILLS, BiomeDictionary.Type.MOUNTAIN);
		BiomeManager.addSpawnBiome(biome);
		BiomeManager.addBiome(BiomeManager.BiomeType.WARM, new BiomeManager.BiomeEntry(biome, 20));
	}
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.4f).scale(1.3f).temperature(0.7f).precipitation(Biome.RainType.NONE)
					.category(Biome.Category.EXTREME_HILLS).waterColor(-6710887).waterFogColor(-6710887).parent("inbetweendimension:inbetween")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.DIORITE.getDefaultState(), Blocks.STONE.getDefaultState(),
							Blocks.STONE.getDefaultState())));
			setRegistryName("inbetween");
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.DIORITE.getDefaultState().getBlock() || ground == Blocks.STONE.getDefaultState().getBlock())
							&& (ground2 == Blocks.DIORITE.getDefaultState().getBlock() || ground2 == Blocks.STONE.getDefaultState().getBlock())))
						return false;
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												Blocks.PLAYER_HEAD.getDefaultState(), bbox);
									}
								}
							}
						}
