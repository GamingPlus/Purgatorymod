...
package net.mcreator.projectwip.world.biome;
...
public class ForestOfSoulsBiome extends InbetweendimensionModElements.ModElement {
	@ObjectHolder("inbetweendimension:forest_of_souls")
	public static final CustomBiome biome = null;
	public ForestOfSoulsBiome(InbetweendimensionModElements instance) {
		super(instance, 50);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		BiomeDictionary.addTypes(biome, BiomeDictionary.Type.FOREST, BiomeDictionary.Type.JUNGLE);
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.JUNGLE).waterColor(4159204).waterFogColor(329011).parent("inbetweendimension:inbetween")
			...
			setRegistryName("forest_of_souls");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Biome.createDecoratedFeature(new CustomTreeFeature(),
					IFeatureConfig.NO_FEATURE_CONFIG, Placement.COUNT_EXTRA_HEIGHTMAP, new AtSurfaceWithExtraConfig(9, 0.1F, 1)));
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.COBBLESTONE.getDefaultState().getBlock() || ground == Blocks.DIORITE.getDefaultState().getBlock())
							&& (ground2 == Blocks.COBBLESTONE.getDefaultState().getBlock()
									|| ground2 == Blocks.DIORITE.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Blocks.DIORITE.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == Blocks.SPRUCE_WOOD.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, Blocks.SPRUCE_WOOD.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = position.getY() - 3 + height; genh <= position.getY() + height; genh++) {
							...
							for (int genx = position.getX() - k4; genx <= position.getX() + k4; genx++) {
								for (int genz = position.getZ() - k4; genz <= position.getZ() + k4; genz++) {
									...
									if (state.isIn(BlockTags.LEAVES) || state.getBlock() == Blocks.SPRUCE_WOOD.getDefaultState().getBlock()) {
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												Blocks.SKELETON_SKULL.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.SPRUCE_WOOD.getDefaultState().getBlock()
					|| blockType == Blocks.SPRUCE_WOOD.getDefaultState().getBlock() || blockType == Blocks.COBBLESTONE.getDefaultState().getBlock()
					|| blockType == Blocks.DIORITE.getDefaultState().getBlock();
		}
