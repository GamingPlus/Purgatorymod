...
package net.mcreator.projectwip.world.structure;
...
public class SpawnStructure extends InbetweendimensionModElements.ModElement {
	public SpawnStructure(InbetweendimensionModElements instance) {
		super(instance, 71);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 30000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("inbetweendimension", "spawn"));
						if (template == null)
							return false;
